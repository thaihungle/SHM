...
class Learner:
    def __init__(self, env_args, train_args, eval_args, policy_args, seed, is_train=True, **kwargs):
        self.seed = seed
        ...
        while self._n_env_steps_total < self.n_env_steps_total:
            ...
            if (
                ...
                if (
                    ...
                    and self._n_env_steps_total > 0.2 * self.n_env_steps_total
    ...
    @torch.no_grad()
    def evaluate2(self, tasks, deterministic=True):
        ...
        for task_idx, task in enumerate(tasks):
            ...
            for episode_idx in tqdm(range(num_episodes)):
                ...
                for stepp in range(num_steps_per_episode):
                    if self.agent_arch == AGENT_ARCHS.Memory:
                        (action, _, _, _), internal_state = self.agent.act(
                            prev_internal_state=internal_state,
                            prev_action=action,
