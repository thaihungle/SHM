import sys, os, time
sys.path.append("./pomdp-baselines")
t0 = time.time()
import socket
import numpy as np
import torch
...
assert seq_model in ["mlp", "lstm", "gru", "lstm-mlp", "gru-mlp", "ffm", "shm", "fwp", "gpt", "s6", "xlstm"]
...
log_folder = os.path.join(exp_id, datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S"))
...
os.system(f"cp -r ./pomdp-baselines/policies/ {log_folder}")
