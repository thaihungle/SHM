...
flags.DEFINE_string("model_path", None, "path to checkpoint file")
...
assert seq_model in ["mlp", "lstm", "gru", "lstm-mlp", "gru-mlp", "ffm", "shm", "fwp", "gpt", "s6"]
...
learner.load_model(FLAGS.model_path)


if learner.env_type == "meta":
     (
    returns_eval,
    success_rate_eval,
    observations_eval,
    total_steps_eval,
    ) = learner.evaluate2(learner.eval_tasks[:1])
elif learner.env_type in ["pomdp", "credit", "atari"]:
    returns_eval, success_rate_eval, _, total_steps_eval = learner.evaluate2(
        learner.eval_tasks[:1]
    )
