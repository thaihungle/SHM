...
class MambaAgent(nn.Module):
	...
	self,
		...
		self.out = nn.Linear(self.input_size, hidden_size)
 
		
		self.core = MambaBlock(input_size,  self.s6_size)
	
	def get_initial_state(self):
		return torch.zeros(1, self.input_size*2,  self.s6_size).to("cuda")
	...
	def forward(self, x, state=None):
		...
		if state is not None:
			...
			state = torch.zeros(1, self.input_size*2,  self.s6_size).to(x.device)
			z, state = self.core(x.transpose(0, 1), state)
		z = self.out(z)
